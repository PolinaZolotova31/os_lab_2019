...
1. Аргументы командной строки (значения, которые передаются скрипту при его вызове из командной строки)
2. Сборка с помощью gcc (clang) (два основных компилятора)
...
2. Системный вызов `fork`(порождается новый процесс (процесс-потомок), который почти идентичен порождающему процессу-родителю)
3. Системный вызов `pipe` (создаёт механизм ввода вывода, который называется конвейером)
...
1. Системный вызов `exec` (изменяет пользовательский контекст текущего процесса, не создавая новый процесс)
