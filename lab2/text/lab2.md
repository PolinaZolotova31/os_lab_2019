...
1. Выделение и освобождение памяти в куче Си. (malloc - выделение, realloc - изменение, free - освобождение)
2. В чем разница между стеком и кучей (прямо в задании не потребуется, но я спрошу). (куча - fifo, стек - lifo)
3. Использование аргументов командной строки (int main(int argc, char *argv[]) argc - кол-во аргументов, *argv[] - аргументы)
...
1. Основы работы компилятора: препроцессор, компилятор, линковщик. Их роли и порядок работы. (Порядок работы: 1 - препроцессор, 2 - компилятор, 3 - линковщик. Препроцессор выполняет предварительную подготовку, компилятор переводит с языка высокого уровня в машинный код, линковщик присоединяет библиотеки)
2. Что такое статическая и динамическая линковка. В чем разница? (Статическая связывается с программой в момент компиляции, Динамическая подключается в момент выполнения)
3. Опции компилятора: `-I, -L, -l, -shared, -o, -с, -fPIC` (-I - хз, -L - путь к каталогу содержащему подключаемые библиотеки, -l - необходимость подключить библиотеку, -shared - указывает, что в результате должен быть собран не исполняемый файл, а динамическая библиотека, -o - выходной файл, -c - компиляция без линковки, -fPIC - позиционно-независимый код, создание динамических библиотек)
4. Утилита ar. (создание архива)
5. Переменная окружения `LD_LIBRARY_PATH` (где искать динамические библиотеки)
...
objdump -x /bin/libsimple.so - выдаст содержимое всех заголовочных файлов, в конце в таблице символов, есть имена функций содержащихся в библиотеке
